...
class HistoryService:
    ...
    def extract_keywords(
        self, text: str, language: str = "en", max_keywords: int = 10
    ) -> List[str]:
        """
    ...
    def categorize_article(self, title: str, content: str, url: str) -> Optional[str]:
        ...
        text_to_check = (
            f"{title_lower} {content_lower[:500]}"  # First 500 chars of content
        )
    ...
    def save_summary_history(
        ...
        try:
            ...
            logger.info(
                f"📝 [HISTORY] Saved summary history for user {user_id}: {article.title}"
            )
    ...
    def save_news_search_history(
        ...
        try:
            ...
            category = self.categorize_article(
                article_title, content_excerpt, article_url
            )
            ...
            logger.info(
                f"📝 [HISTORY] Saved news search history for user {user_id}: {article_title}"
            )
            return history_entry
    ...
    def get_user_history(
        ...
        try:
            ...
            history_items = (
                query.order_by(desc(UserHistory.created_at))
                .offset(offset)
                .limit(per_page)
                .all()
            )
            ...
            logger.info(
                f"📖 [HISTORY] Retrieved {len(history_items)} history items for user {user_id}"
            )
    ...
    def get_user_keywords(
        self, db: Session, user_id: str, limit_days: int = 30
    ) -> List[str]:
        """
        ...
        try:
            ...
            logger.info(
                f"🔍 [HISTORY] Found {len(top_keywords)} frequent keywords for user {user_id}"
            )
            return top_keywords
    ...
    def get_user_categories(
        ...
        try:
            ...
            top_categories = [
                category for category, freq in category_freq.most_common(10)
            ]

            logger.info(
                f"📂 [HISTORY] Found {len(top_categories)} frequent categories for user {user_id}"
            )
    ...
    def generate_keyword_based_recommendations(
        ...
        try:
            ...
            logger.error(
                f"❌ [HISTORY] Failed to generate keyword-based recommendations: {e}"
            )
            return []
    ...
    def generate_category_based_recommendations(
        ...
        try:
            ...
            for article in available_articles:
                article_category = self.categorize_article(
                    article.title, article.content, str(article.url)
                )
                ...
                if article_category and article_category in user_categories:
                    ...
                    score = (
                        1.0 / category_rank
                    )  # Higher score for more frequent categories
            ...
            logger.error(
                f"❌ [HISTORY] Failed to generate category-based recommendations: {e}"
            )
            return []
    ...
    def generate_recommendations(
        ...
        try:
            ...
            keyword_recs = self.generate_keyword_based_recommendations(
                db, user_id, available_articles, max_total // 2
            )
            all_recommendations.extend(keyword_recs)

            # Get category-based recommendations
            category_recs = self.generate_category_based_recommendations(
                db, user_id, available_articles, max_total // 2
            )
            ...
            logger.info(
                f"🎯 [HISTORY] Generated {len(result)} recommendations for user {user_id}"
            )
            return result
    ...
    def log_recommendation_click(self, db: Session, user_id: str, article_url: str):
        ...
        try:
            ...
            if rec_log:
                ...
                db.commit()
                logger.info(
                    f"👆 [HISTORY] Logged recommendation click for user {user_id}: {article_url}"
                )
    ...
    def get_user_stats(self, db: Session, user_id: str) -> Dict:
        ...
        try:
            # Get total summaries
            total_summaries = (
                db.query(UserHistory).filter(UserHistory.user_id == user_id).count()
            )
