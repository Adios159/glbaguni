...
class Settings(BaseSettings):
    ...
    # 기본 애플리케이션 설정
    app_name: str = Field(default="글바구니 뉴스 요약 서비스", description="애플리케이션 이름")
    ...
    # 데이터베이스 설정
    database_url: str = Field(default="sqlite:///glbaguni.db", description="데이터베이스 URL")
    ...
    # 파일 업로드 설정
    upload_max_size: int = Field(default=10*1024*1024, description="업로드 최대 크기 (바이트)")
    upload_allowed_extensions: List[str] = Field(
        default=["txt", "json", "csv"],
        description="허용된 업로드 파일 확장자"
    )
...
def validate_required_env_vars(settings: Settings) -> None:
    ...
    # 필수 변수 목록
    required_checks = [
        ("OPENAI_API_KEY", settings.openai_api_key, "OpenAI API 키가 설정되지 않았습니다"),
        ("SECRET_KEY", settings.secret_key, "애플리케이션 비밀 키가 설정되지 않았습니다"),
    ]
    ...
    if settings.openai_api_key and not settings.openai_api_key.startswith("sk-"):
        missing_vars.append("❌ OPENAI_API_KEY: OpenAI API 키 형식이 올바르지 않습니다 (sk-로 시작해야 함)")
...
def get_settings() -> Settings:
    ...
    try:
        ...
        logger.info(f"💾 데이터베이스: {'SQLite' if settings.database_is_sqlite else 'Other'}")
