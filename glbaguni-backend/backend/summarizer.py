...
class ArticleSummarizer:
    ...
    def __init__(self):
        ...
        self.default_prompt = settings.SUMMARIZATION_PROMPT
    ...
    def summarize_article(self, article: Article, custom_prompt: Optional[str] = None, language: str = "en") -> Optional[ArticleSummary]:
        ...
        try:
            ...
            logger.info(f"🌐 [OPENAI] Target language: {'Korean' if language == 'ko' else 'English'} - {article.title}")
            logger.info(f"📝 [OPENAI] Content length after truncation: {len(content)} characters")
            ...
            logger.info(f"💬 [OPENAI] Using {'Korean' if language == 'ko' else 'English'} system prompt")
    ...
    def summarize_articles(self, articles: List[Article], 
                          custom_prompt: Optional[str] = None, language: str = "en") -> List[ArticleSummary]:
        ...
        logger.info(f"🚀 [SUMMARIZER] Starting batch summarization for {len(articles_to_process)} articles")
        logger.info(f"🌐 [SUMMARIZER] Target language: {'Korean' if language == 'ko' else 'English'}")
        ...
        for i, article in enumerate(articles_to_process):
            ...
            if time.time() - start_time > max_total_time:
                logger.warning(f"⏰ [SUMMARIZER] Total time limit exceeded, stopping at {i} articles")
                break
            
            article_start_time = time.time()
            
            try:
                logger.info(f"📄 [SUMMARIZER] Processing {i+1}/{len(articles_to_process)}: {article.title[:50]}...")
                ...
                if article_processing_time > max_per_article:
                    logger.warning(f"⏰ [SUMMARIZER] Article processing took too long: {article_processing_time:.2f}s")
                
                if summary:
                    summaries.append(summary)
                    successful_summaries += 1
                    logger.info(f"✅ [SUMMARIZER] Summary {i+1} completed in {article_processing_time:.2f}s")
                else:
                    failed_summaries += 1
        ...
        logger.info(f"   📈 Success rate: {(successful_summaries / len(articles_to_process) * 100):.1f}%")
    ...
    def _is_korean_content(self, content: str) -> bool:
        ...
        korean_chars = len(re.findall(r'[\u1100-\u11FF\u3130-\u318F\uAC00-\uD7AF]', content))
    ...
    def _get_system_message(self, is_korean: bool) -> str:
        ...
        if is_korean:
            return ("당신은 한국어 뉴스 기사를 정확하고 간결하게 요약하는 전문 어시스턴트입니다. "
                   "핵심 정보를 명확하게 전달하고, 객관적인 톤을 유지하며, "
                   "중요한 사실과 맥락을 놓치지 않고 요약해 주세요.")
        else:
            return ("You are a helpful assistant that summarizes news articles concisely and accurately. "
