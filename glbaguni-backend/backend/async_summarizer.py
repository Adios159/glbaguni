...
class AsyncArticleSummarizer:
    ...
    async def summarize(self, text: str, language: str = "ko", max_retries: int = None) -> Dict[str, Any]:
        ...
        for attempt in range(retries):
            try:
                ...
                logger.warning(f"⏳ [{req_id}] Rate limit 도달, {wait_time}초 대기 후 재시도")
    ...
    def _build_messages(self, text: str, language: str) -> List[ChatCompletionMessageParam]:
        """언어별 메시지 구성"""
        if language == "ko":
            system_message = (
                "너는 뉴스 기사를 요약하는 전문가야. "
    ...
    async def summarize_articles(
        ...
        async def summarize_with_semaphore(article: Article) -> Optional[ArticleSummary]:
    ...
    async def summarize_with_keywords(
        ...
        try:
            ...
            if isinstance(summary_result, dict) and "summary" in summary_result:
                ...
                result["error"] = str(summary_result) if isinstance(summary_result, Exception) else "요약 오류"
    ...
    async def _extract_keywords(self, text: str, language: str = "ko") -> List[str]:
        ...
        try:
            if language == "ko":
                ...
                user_message = f"다음 텍스트에서 핵심 키워드를 추출해줘:\n\n{text[:1000]}..."
            else:
                system_message = (
                ...
                user_message = f"Extract key keywords from the following text:\n\n{text[:1000]}..."
    ...
    async def get_summary_stats(self, summaries: List[ArticleSummary]) -> Dict[str, Any]:
        """요약 통계 정보 생성 (비동기)"""
        if not summaries:
            return {
                "total_articles": 0,
...
async def summarize_articles_async(
    ...
    """기사 목록 비동기 요약 (편의 함수)"""
    summarizer = AsyncArticleSummarizer()
    return await summarizer.summarize_articles(articles, language, max_concurrent=max_concurrent)