...
try:
    ...
    from backend.services.gpt import get_gpt_service, safe_gpt_call
except ImportError:
    try:
        from config import settings
        from models import Article, ArticleSummary
        from services.gpt import get_gpt_service, safe_gpt_call
    except ImportError:
        # Fallback imports
        import logging

        async def safe_gpt_call(
            prompt: str, language: str = "ko", max_retries: int = 3
        ) -> str:
...
class ArticleSummarizer:
    ...
    async def summarize(self, input_text: str, language: str = "ko") -> Dict[str, Any]:
        ...
        try:
            ...
            if len(input_text) > self.max_content_length:
                ...
                logger.warning(
                    f"Text truncated to {self.max_content_length} characters"
                )

            logger.info(
                f"Summarizing text - Length: {len(input_text)}, Language: {language}"
            )
    ...
    async def summarize_article(
        ...
        try:
            ...
            logger.info(
                f"Summarizing article: {article.title[:50]}... (Language: {language})"
            )
