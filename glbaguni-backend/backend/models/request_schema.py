...
class SummarizeRequest(BaseModel):
    ...
    rss_urls: List[str] = Field(
        ...
        min_items=1, 
        max_items=10,
        description="RSS 피드 URL 목록 (최대 10개)"
    )
    ...
    max_articles: int = Field(
        ...
        ge=1, 
        le=50,
        description="수집할 최대 기사 수 (1-50)"
    )
    
    summary_style: str = Field(
        default="compact",
        description="요약 스타일 (compact, detailed, bullet)"
    )
    
    language: str = Field(
        default="ko",
        description="요약 언어 (ko, en)"
    )
    
    filter_keywords: Optional[List[str]] = Field(
        default=None,
        max_items=20,
        description="필터링할 키워드 목록 (포함할 키워드)"
    )
    ...
    user_email: Optional[str] = Field(
        default=None,
        description="결과 전송용 이메일 주소"
    )
    
    save_to_history: bool = Field(
        default=True,
        description="히스토리 저장 여부"
    )
...
class NewsSearchRequest(BaseModel):
    ...
    sort_by: str = Field(
        default="relevance",
        description="정렬 기준 (relevance, date, popularity)"
    )
...
class RecommendationRequest(BaseModel):
    ...
    user_interests: List[str] = Field(
        ...
        min_items=1,
        max_items=20,
        description="사용자 관심사 키워드"
    )
    
    preferred_sources: Optional[List[str]] = Field(
        default=None,
        max_items=10,
        description="선호하는 뉴스 소스"
    )
