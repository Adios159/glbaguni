...
def validate_user_input(text: str, max_length: int = 5000) -> str:
    ...
    for pattern in dangerous_patterns:
        if re.search(pattern, text, re.IGNORECASE):
            raise HTTPException(
                status_code=400, detail="허용되지 않는 문자패턴이 감지되었습니다."
            )
...
def validate_and_sanitize_text(
    text: str, max_length: int = 5000, min_length: int = 10
) -> str:
    """텍스트 입력 검증 및 정제"""
    if not text or not text.strip():
        raise HTTPException(status_code=400, detail="입력 텍스트가 비어있습니다.")
    ...
    for pattern in malicious_patterns:
        if re.search(pattern, text, re.IGNORECASE):
            raise HTTPException(
                status_code=400, detail="허용되지 않는 스크립트 패턴이 감지되었습니다."
            )
...
def validate_url(url: Union[str, HttpUrl]) -> str:
    ...
    try:
        ...
        if parsed.scheme not in ["http", "https"]:
            raise HTTPException(
                status_code=400, detail="HTTP 또는 HTTPS URL만 허용됩니다."
            )
    except Exception:
...
def validate_urls(urls: List[Union[str, HttpUrl]], max_count: int = 20) -> List[str]:
    ...
    for i, url in enumerate(urls):
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"URL {i+1}번 검증 실패: {e.detail}"
            )
...
def validate_positive_integer(
    value: Optional[int], default: int, max_value: int = 100
) -> int:
    """양의 정수 검증"""
